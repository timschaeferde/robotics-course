...
def set_frame_properties(obj: ry.Frame, shape=None, size=None, color=None, position=None,
    ...
    if shape is not None and size is not None: obj.setShape(shape, size)
    if color is not None: obj.setColor(color / 255.)  # Rai need normalized float colors
    if position is not None: obj.setPosition(position)
    if quaternion is not None: obj.setQuaternion(quaternion)
    if mass is not None: obj.setMass(mass)
    return obj
